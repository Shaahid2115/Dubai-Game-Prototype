...
namespace Game.Modules.Utility
{
    public sealed class UtilityModule : Module<UtilityModuleView>
    {
        ...
        private readonly List<InventoryController> _playerInventories;
        ...
        public UtilityModule(UtilityModuleView view) : base(view)
        {
            ...
            _playerInventories = new List<InventoryController>();
        ...
        private void OnTick()
        {
            ...
            int index = 0;
            foreach (var inventory in _playerInventories.ToList())
            {
        ...
        private void OnPlayerIdle()
        {
            ...
            if (item.Type == ItemType.GetInventory && _playerInventories.Count < _config.PlayerInventoriesMax)
            {
                ...
                Vector3 endPosition = _gameManager.Player.GetInventoryPosition(_playerInventories.Count);
                inventory.Fly(endPosition, _inventoryFlyTime);
                inventory.ON_FLY_END += OnPlayerGetInventory;
            }
            else if (item.Type == ItemType.DropInventory && _playerInventories.Count > 0)
            {
                int index = _playerInventories.Count - 1;
                var inventory = _playerInventories[index];

                _gameManager.Model.InventoryTypes.Remove(inventory.Type);
                _gameManager.Model.Save();

                _playerInventories.Remove(inventory);
        ...
        private void OnPlayerGetInventory(InventoryController inventory)
        {
            ...
            _playerInventories.Add(inventory);
        }
